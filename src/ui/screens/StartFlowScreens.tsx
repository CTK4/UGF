...
export function StaffMeetingScreen({ ui }: ScreenProps) {
  const state = ui.getState();
  const save = state.save;
  const existing = save?.gameState.offseasonPlan ?? { priorities: [], resignTargets: [], shopTargets: [], tradeNotes: "" };

  const PRIORITY_OPTIONS = [
    "Re-sign Core Players",
    "Add Starter Talent in Free Agency",
    "Shop Veterans for Picks",
    "Upgrade Coaching Staff",
    "Improve Team Speed",
    "Improve Pass Protection",
  ] as const;

  const [priorities, setPriorities] = React.useState<string[]>(existing.priorities ?? []);
  const [resignTargetsRaw, setResignTargetsRaw] = React.useState<string>((existing.resignTargets ?? []).join(", "));
  const [shopTargetsRaw, setShopTargetsRaw] = React.useState<string>((existing.shopTargets ?? []).join(", "));
  const [tradeNotes, setTradeNotes] = React.useState<string>(String(existing.tradeNotes ?? ""));
  ...
  function submit() {
    ui.dispatch({
      ...
      payload: {
        priorities,
        resignTargets: parseCsv(resignTargetsRaw),
        shopTargets: parseCsv(shopTargetsRaw),
        tradeNotes,
      },
    });
  }
  ...
  return (
    <div className="ugf-card" data-ui={UI_ID.staffMeeting.root}>
      <div className="ugf-card__header">
        <h2 className="ugf-card__title">Staff Meeting</h2>
      </div>
      <div className="ugf-card__body" style={{ display: "grid", gap: 14 }}>
        <div className="ugf-pill">
          Submit offseason priorities to clear the advance blocker. Keep it high level—this becomes your offseason plan.
        </div>

        <div className="ugf-card" data-ui={UI_ID.staffMeeting.priorities}>
          <div className="ugf-card__body" style={{ display: "grid", gap: 10 }}>
            <div style={{ fontWeight: 700 }}>Priorities</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(220px, 1fr))", gap: 8 }}>
              {PRIORITY_OPTIONS.map((label) => {
                const checked = priorities.includes(label);
                return (
                  <label
                    key={label}
                    data-ui={UI_ID.staffMeeting.priorityOption}
                    style={{ display: "flex", gap: 10, alignItems: "center", padding: 10, borderRadius: 10, border: "1px solid rgba(255,255,255,0.14)" }}
                  >
                    <input type="checkbox" checked={checked} onChange={() => togglePriority(label)} />
                    <span>{label}</span>
                  </label>
                );
              })}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(220px, 1fr))", gap: 10 }}>
          <label style={{ display: "grid", gap: 6 }}>
            <span style={{ fontWeight: 700 }}>Re-sign Targets</span>
            <input
              data-ui={UI_ID.staffMeeting.resignTargets}
              value={resignTargetsRaw}
              onChange={(e) => setResignTargetsRaw(e.target.value)}
              placeholder="Comma-separated names (e.g. John Smith, Alex Brown)"
            />
          </label>
          <label style={{ display: "grid", gap: 6 }}>
            <span style={{ fontWeight: 700 }}>Shop Targets</span>
            <input
              data-ui={UI_ID.staffMeeting.shopTargets}
              value={shopTargetsRaw}
              onChange={(e) => setShopTargetsRaw(e.target.value)}
              placeholder="Comma-separated names (e.g. Veteran WR, Backup QB)"
            />
          </label>
        </div>

        <label style={{ display: "grid", gap: 6 }}>
          <span style={{ fontWeight: 700 }}>Trade Notes</span>
          <textarea
            data-ui={UI_ID.staffMeeting.tradeNotes}
            value={tradeNotes}
            onChange={(e) => setTradeNotes(e.target.value)}
            placeholder="Optional notes for the front office..."
            rows={4}
          />
        </label>

        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <button className="primary" type="button" data-ui={UI_ID.staffMeeting.submit} onClick={submit}>
            Submit Priorities
          </button>
          <button type="button" data-ui={UI_ID.staffMeeting.backToHub} onClick={() => ui.dispatch({ type: "NAVIGATE", route: { key: "Hub" } })}>
            Back to Hub
          </button>
        </div>
      </div>

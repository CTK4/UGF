...
export function FreeAgencyScreen({ ui }: ScreenProps) {
  ...
  if (!save) {
    return <div className="ugf-card"><div className="ugf-card__body">No active save.</div></div>;
  }
  ...
  return (
    <div className="ugf-card">
      ...
      <div className="ugf-card__body" style={{ display: "grid", gap: 10 }}>
        <div className="ugf-pill">Cap Space: <b>{money(cap.capSpace)}</b> · Payroll: {money(cap.payroll)} · Cap Limit: {money(cap.capLimit)}</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(150px, 1fr))", gap: 8 }}>
          <label>Position<select value={position} onChange={(event) => setPosition(event.target.value)}>{positions.map((pos) => <option key={pos} value={pos}>{pos}</option>)}</select></label>
          <label>Min OVR<input type="number" min={40} max={99} value={minOverall} onChange={(event) => setMinOverall(Number(event.target.value) || 40)} /></label>
          <label>Search<input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Player name" /></label>
        </div>

        <div style={{ display: "grid", gap: 8 }}>
          {!filtered.length ? <div className="ugf-pill">No matching free agents.</div> : null}
          {filtered.map((row) => (
            <div key={row.id} className="ugf-card">
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button onClick={() => ui.dispatch({ type: "REFRESH_FREE_AGENCY" })}>Refresh Pool</button>
          <button onClick={() => ui.dispatch({ type: "NAVIGATE", route: { key: "Hub", tab: "roster" } })}>Back to Roster</button>
        </div>
      </div>
    </div>
  );

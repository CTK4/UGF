...
export function PhoneThreadScreen({ ui }: ScreenProps) {
  ...
  const thread = save.phone.threads.find((t) => t.id === state.route.threadId);
    ...
    <div className="ugf-card">
      ...
      <div className="ugf-card__body" style={{ display: "grid", gap: 8 }}>
        {thread.messages.map((m) => (
          <div key={m.id} className="ugf-card">
            <div className="ugf-card__body"><b>{m.from}:</b> {m.text}</div>
          </div>
        ))}
        <button onClick={() => ui.dispatch({ type: "NAVIGATE", route: { key: "PhoneInbox" } })}>Back to Inbox</button>
      </div>
    </div>
  );
}

  ...
  return (
    <div className="app">
      <header className="ugf-topbar">
        ...
        {(() => {
          const canGo = (key: RouteKey) => !(saveRequiredRoutes.has(key) && noSave);
          const dispatchAndClose = (action: any) => {
            setNavOpen(false);
            ui.dispatch(action);
          };
          const NavButtons = (
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end" }}>
              <button onClick={() => dispatchAndClose({ type: "NAVIGATE", route: { key: "Start" } })}>Start</button>
              <button disabled={!canGo("Hub")} onClick={() => dispatchAndClose({ type: "NAVIGATE", route: { key: "Hub" } })}>Hub</button>
              <button disabled={!canGo("Roster")} onClick={() => dispatchAndClose({ type: "NAVIGATE", route: { key: "Roster" } })}>Roster</button>
              <button disabled={!canGo("FreeAgency")} onClick={() => dispatchAndClose({ type: "NAVIGATE", route: { key: "FreeAgency" } })}>Free Agency</button>
              <button disabled={!canGo("StaffTree")} onClick={() => dispatchAndClose({ type: "NAVIGATE", route: { key: "StaffTree" } })}>Staff</button>
              <button disabled={!canGo("PhoneInbox")} onClick={() => dispatchAndClose({ type: "NAVIGATE", route: { key: "PhoneInbox" } })}>Phone</button>
              <button className="danger" onClick={() => dispatchAndClose({ type: "RESET_SAVE" })}>Reset Save</button>
            </div>
          );
